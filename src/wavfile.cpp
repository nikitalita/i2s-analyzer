...
int write_PCM_stereo_header(FILE *file_p,
                                int32_t SampleRate,
                                int32_t FrameCount,
                                int8_t NumChannels,
                                int8_t BitsPerSample)
{
    ...
    const int32_t subchunk2_size = FrameCount * NumChannels * BitsPerSample / 8;
    const int32_t chunk_size = 4 + (8 + SUBCHUNK1SIZE) + (8 + subchunk2_size);
    const int32_t byte_rate = SampleRate * NumChannels * BitsPerSample / 8;
    const int32_t block_align = NumChannels * BitsPerSample / 8;
    size_t write_count;
