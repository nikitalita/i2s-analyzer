...
class WavWriter {
    ...
    public:
        ...
        bool initialize(const char* writeFilePath,
                        ...
                        uint8_t bitsPerSample); //Number of bytes required to represent the value of a single channel of a sample
        
        bool startWriting();

        bool writeSample(uint64_t sample1, uint64_t sample2);
    ...
    private:
        ...
        uint8_t bitsPerSample; //Number of bits per sample (may be non-8-multiple)
        bool initialized;
